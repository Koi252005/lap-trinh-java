...
export default function ProductTraceabilityPage() {
  const { id } = useParams();
  const [product, setProduct] = useState<ProductTrace | null>(null);
  ...
  useEffect(() => {
    if (id) {
      axios
        .get(`${API_BASE}/public/traceability/product/${id}`, { validateStatus: () => true })
        .then((res) => {
  ...
  if (error || !product) return <div className="p-8 text-center text-red-600">{error || 'Không tìm thấy sản phẩm.'}</div>;
      ...
      <div className="max-w-3xl mx-auto bg-white dark:bg-gray-800 rounded-xl shadow-2xl overflow-hidden">
        ...
        <div className="p-8">
          <div className="mb-8 border-b border-gray-100 dark:border-gray-700 pb-8">
            <h2 className="text-xl font-bold text-gray-800 dark:text-white mb-4">📦 Thông Tin Sản Phẩm</h2>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div>
                ...
                <p className="font-semibold text-gray-800 dark:text-gray-200">{product.batchCode || '—'}</p>
              </div>
            </div>
          </div>
          ...
          <div className="mb-8 border-b border-gray-100 dark:border-gray-700 pb-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                ...
                <p className="font-semibold text-gray-800 dark:text-gray-200">{product.farm?.name}</p>
              </div>
              <div>
                <p className="text-sm text-gray-500">Địa chỉ</p>
                <p className="font-semibold text-gray-800 dark:text-gray-200">{product.farm?.address}</p>
              </div>
              <div>
                <p className="text-sm text-gray-500">Chứng nhận</p>
                <p className="font-semibold text-green-600">{product.farm?.certification || 'Chưa cập nhật'}</p>
              </div>
            </div>
          </div>
          ...
          {season && (
            <>
              ...
              {season.processes && season.processes.length > 0 && (
                <div className="mb-8">
